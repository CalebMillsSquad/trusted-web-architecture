...
export async function POST(request: Request) {
  ...
  if (process.env.AI_LEAD_TEST_MODE !== "true") return NextResponse.json({ error: "Secure lead storage and confirmed email delivery are not configured. Please use the direct email option." }, { status: 503 });
