...
Production readiness: canonical metadata, structured data, `robots.txt`, `sitemap.xml`, and conservative security headers are generated by Next.js. No analytics or error-monitoring provider is enabled. The assistant fails safely when the OpenAI project has no quota. Lead intake remains unavailable until approved durable storage and notification delivery are configured; appointments are never represented as confirmed without a calendar connection.
